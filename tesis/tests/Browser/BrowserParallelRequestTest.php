...
class BrowserParallelRequestTest extends DuskTestCase
{
    ...
    public function a_subjects_selection_cannot_be_empty_Parallel()
    {
        ...
        $this->browse(function (Browser $browser) use($user,$petition) {
            ...
            $browser->visit(route('request.verify',['ID'   =>  $ID_crypt,'code' =>  $code_crypt]))
                    ->assertSee('Formulario para solicitar materias en paralelo')
                    ->attach('proof_notes',\Config::get('constants.proof_notes'))
                    ->attach('reason_letter',\Config::get('constants.reason_letter_Parallel'))
                    ->press('#Enviar')
                    ->assertSee('Formulario para solicitar materias en paralelo')
                    ->assertUrlIs(route('request.parallel',['ID'   =>  $ID_crypt,'code' =>  $code_crypt]));
    ...
    public function a_subjects_selection_cannot_be_not_PDF_Parallel()
    {
        //1. Given -> Teniendo un usuario
        $user = factory(User::class)->create([
            'area' => random_int(1,3)
    ...
    public function a_proof_notes_cannot_be_not_PDF_Parallel()
    {
        //1. Given -> Teniendo un usuario
        $user = factory(User::class)->create([
            'area' => random_int(1,3)
    ...
    public function a_reason_letter_cannot_be_not_PDF_Parallel()
    {
        //1. Given -> Teniendo un usuario
        $user = factory(User::class)->create([
            'area' => random_int(1,3)
    ...
    public function an_user_can_complete_Parallel_Request()
    {
        //1. Given -> Teniendo un usuario
        $user = factory(User::class)->create([
            'area' => random_int(1,3)
